...
describe("directory departments config route", () => {
  ...
  it("forwards only diagnosis session authorization to the backend", async () => {
    const response = await GET(
      new Request(
        "https://console.example.com/api/config/directory/departments?provider=ops_iam&limit=3",
        {
          headers: {
    ...
    expect(url.toString()).toBe(
      "https://api.example.com/api/v1/config/directory/departments?limit=3&provider=ops_iam",
    );
  ...
  it("requires explicit or browser-session authorization", async () => {
    const response = await GET(
      new Request("https://console.example.com/api/config/directory/departments"),
    );
  ...
  it("rejects invalid limits before contacting the backend", async () => {
    const response = await GET(
      new Request(
        "https://console.example.com/api/config/directory/departments?limit=0",
        {
          headers: {
    ...
    await expect(response.json()).resolves.toEqual({
      error: "Directory list limit must be a positive integer.",
    });
    expect(fetch).not.toHaveBeenCalled();
  });
});
