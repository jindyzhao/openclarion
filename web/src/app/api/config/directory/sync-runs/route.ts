import { fetchDirectorySyncRuns } from "@/features/settings/directory-rbac/api";
...
export async function GET(request: Request) {
  ...
  return protectedApiResultResponse(
    request,
    await fetchDirectorySyncRuns(options.data, { headers: headers.data }),
  );
}
...
function listLimit(value: string | null): ApiResult<number | undefined> {
  ...
  return {
    ok: false,
    error: {
      message: "Directory sync run list limit must be a positive integer.",
      status: 400,
    },
  };
}
