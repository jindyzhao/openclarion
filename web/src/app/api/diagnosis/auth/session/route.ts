...
export async function POST(request: Request) {
  const authorization = normalizeForwardedAuthorization(request.headers.get("authorization") ?? "");
  if (authorization === null) {
    return NextResponse.json<ErrorResponse>({ error: "authorization is required" }, { status: 401 });
  }
  const result = await requestJSON<unknown>("/api/v1/diagnosis/auth/session", {
    method: "POST",
  ...
  if (session === null) {
    return NextResponse.json<ErrorResponse>({ error: "diagnosis browser session response is invalid" }, { status: 502 });
  }
...
export async function GET(request: Request) {
  if (!diagnosisRequestHasSessionCookie(request)) {
    return NextResponse.json<DiagnosisSessionStatusResponse>({ authenticated: false });
  }
  const authorization = diagnosisAuthorizationFromRequest(request);
  if (authorization === null) {
    const response = NextResponse.json<DiagnosisSessionStatusResponse>({ authenticated: false });
