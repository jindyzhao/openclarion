...
import type { DiagnosisOIDCTokenResponse } from "@/lib/api/diagnosis-oidc-login";
import {
...
export async function GET(request: Request) {
  ...
  if (requestURL.searchParams.has("error")) {
    return oidcCallbackFailureRedirect(
      request,
      sealedState.returnTo,
      "oidc_auth_failed",
    );
  }
  const code = oidcCallbackQueryToken(
    requestURL.searchParams,
    "code",
    oidcCallbackCodeMaxBytes,
  );
  const state = oidcCallbackQueryToken(
    requestURL.searchParams,
    "state",
    oidcCallbackStateMaxBytes,
  );
  ...
  const token = await exchangeOIDCCodeForIDToken({
    code,
    config,
    payload: sealedState,
  });
  ...
  if (token.accessToken !== undefined) {
    sessionHeaders.set("x-openclarion-oidc-access-token", token.accessToken);
  }
  const sessionResult = await requestJSON<unknown>(
  ...
  if (!sessionResult.ok) {
    return oidcCallbackFailureRedirect(
      request,
      sealedState.returnTo,
      oidcCallbackFailureError(sessionResult.error.status),
    );
  }
  const session = normalizedDiagnosisAuthSessionResponse(sessionResult.data);
  if (session === null) {
    return NextResponse.json<ErrorResponse>(
      { error: "OIDC session response is invalid" },
      { status: 502 },
    );
  }
  ...
  const requestOrigin = diagnosisRequestPublicOrigin(request);
...
async function exchangeOIDCCodeForIDToken({
  ...
  const tokenRequest = oidcTokenEndpointRequest({
    code,
    config,
    discovery,
    payload,
...
function oidcCallbackFailureError(
  ...
  | "oidc_role_unauthorized" {
  if (status === 403) {
    return "oidc_role_unauthorized";
  }
  if (status === 401) {
    return "oidc_auth_failed";
  }
