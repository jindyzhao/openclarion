		...
		func(_ context.Context, got temporalpkg.DiagnosisTurnActivityInput) (temporalpkg.DiagnosisTurnActivityResult, error) {
			...
			if got.MessageID == "msg-1" {
				...
				return temporalpkg.DiagnosisTurnActivityResult{
					...
					RawOutput:           json.RawMessage(`{"schema_version":"diagnosis_turn.v1","message":"` + message + `","confidence":"low","requires_human_review":true,"confidence_rationale":"Current evidence is insufficient to raise confidence.","missing_evidence_requests":[{"label":"Current alert context","detail":"Collect current sibling alerts before finalizing.","priority":"high"}],"conclusion_status":"needs_evidence"}`),
