...
require_direct_directory() {
  local path="$1"
  if [[ -L "$path" ]]; then
    echo "[adr-check] $path must be a real directory, not a symlink." >&2
    return 1
  fi
  if [[ ! -d "$path" ]]; then
    echo "[adr-check] $path must be a real directory." >&2
    return 1
  fi
}
...
mapfile -t indirect_adr_files < <(find "$adr_dir" -maxdepth 1 -name 'ADR-*.md' ! -type f | sort)
for f in "${indirect_adr_files[@]}"; do
  require_regular_file "$f" || failed=1
done
