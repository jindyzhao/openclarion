#!/usr/bin/env bash
# Build the local M4 report-enhancer runtime and leave an immutable digest
# reference in the Docker daemon. No Kubernetes or external registry is needed.
...
fail() {
  echo "[report-enhancer-runner-build] $1" >&2
  exit 2
}

target_arch="${OPENCLARION_REPORT_ENHANCER_RUNNER_TARGET_ARCH:-amd64}"
case "$target_arch" in
  amd64 | arm64) ;;
  *) fail "target architecture must be amd64 or arm64" ;;
esac
target_platform="linux/${target_arch}"
registry_ready_timeout_seconds="${OPENCLARION_REPORT_ENHANCER_RUNNER_REGISTRY_READY_TIMEOUT_SECONDS:-30}"
...
validate_output_file() {
  ...
  path_lexical="$(realpath -ms -- "$path")" || fail "digest ref output path must be resolvable"
  path_part="$(dirname -- "$path_lexical")"
  while [[ "$path_part" != "/" && "$path_part" != "." ]]; do
    [[ ! -L "$path_part" ]] || fail "digest ref output parent must not be a symlink: $path_part"
...
output_file="${OPENCLARION_REPORT_ENHANCER_RUNNER_DIGEST_REF_OUT:-$ROOT_DIR/.openclarion-private/report-enhancer-runner.digest-ref}"
...
run_id="${OPENCLARION_REPORT_ENHANCER_RUNNER_BUILD_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
if [[ ! "$run_id" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$ ]]; then
  fail "build id must match [A-Za-z0-9][A-Za-z0-9._-]{0,79}"
fi

registry_name="openclarion-report-enhancer-registry-${run_id}"
local_tag="openclarion/report-enhancer-runner:local-${run_id}"
registry_image="${OPENCLARION_REPORT_ENHANCER_REGISTRY_IMAGE:-registry:2@sha256:a3d8aaa63ed8681a604f1dea0aa03f100d5895b6a58ace528858a7b332415373}"
if [[ ! "$registry_image" =~ ^[^[:space:]@]+@sha256:[a-f0-9]{64}$ ]]; then
  fail "registry image must be an immutable lowercase digest reference"
fi
...
tmp_dir="$(mktemp -d -t openclarion-report-enhancer.XXXXXX)"
registry_cid=""
remote_tag=""
tmp_output=""
...
echo "[report-enhancer-runner-build] building static binary for ${target_platform}..." >&2
CGO_ENABLED=0 GOOS=linux GOARCH="$target_arch" GOWORK=off \
  go build -trimpath -ldflags='-s -w' \
  -o "$tmp_dir/report-enhancer-runner" ./scripts/report_enhancer_runner
chmod 0755 "$tmp_dir/report-enhancer-runner"
echo "[report-enhancer-runner-build] collecting third-party license files..." >&2
GOWORK=off go run github.com/google/go-licenses@v1.6.0 save \
  ...
  ./scripts/report_enhancer_runner
cp scripts/report_enhancer_runner/Dockerfile "$tmp_dir/Dockerfile"
cp scripts/report_enhancer_runner/THIRD_PARTY_NOTICES.txt "$tmp_dir/THIRD_PARTY_NOTICES.txt"
...
echo "[report-enhancer-runner-build] resolving immutable repository digest..." >&2
...
repository="localhost:${host_port}/openclarion/report-enhancer-runner"
...
mkdir -p "$output_dir"
tmp_output="$(mktemp "$output_dir/.report-enhancer-runner.digest-ref.XXXXXX")"
chmod 0600 "$tmp_output"
printf '%s\n' "$digest_ref" >"$tmp_output"
mv -f "$tmp_output" "$output_file"
tmp_output=""

echo "[report-enhancer-runner-build] local immutable image is ready." >&2
echo "[report-enhancer-runner-build] image platform: $target_platform" >&2
echo "[report-enhancer-runner-build] digest ref file: $output_file" >&2
printf '%s\n' "$digest_ref"
