...
| OpenAPI -> TS type freshness | M3 | landed | `make openapi-ts-fresh`; requires `web/`, `api/openapi.yaml`, `web/package.json`, and `web/src/lib/api/openapi.ts` to stay direct regular inputs, regenerates frontend API types via `openapi-typescript`, and fails on diff |
