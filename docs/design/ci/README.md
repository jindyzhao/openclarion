...
| Forbidden oapi-codegen v2 / openapi.compat.yaml | M0 -> M3 hardening | landed | `make forbidden-oapi-v2`; rejects `github.com/oapi-codegen/oapi-codegen/v2` in `go.mod` or Go imports and rejects any `api/openapi.compat.yaml` path, including regular files, directories, symlinks, and dangling symlinks. `go test ./scripts` covers native-V3 acceptance plus compat-path, module, and source-import failures. |
