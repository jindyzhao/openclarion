...
| Forbidden SQLite in Go tests | M0 | landed | `make forbidden-sqlite`; fixture-backed script tests cover no-test skip, allowed Go tests, SQLite import/DSN rejection, non-test Go exclusion, and symlink/non-regular `_test.go` fail-closed behavior |
