...
| OpenAPI lint (`vacuum lint --fail-severity error`) | M0 | landed | `make openapi-lint`; vacuum is a `go tool` dependency so the gate is hermetic; the gate rejects symlink/non-regular OpenAPI spec and vacuum ruleset inputs before linting, and the ruleset lives at `docs/design/ci/vacuum/.vacuum.yaml` to explicitly accept OpenClarion's snake_case JSON convention |
