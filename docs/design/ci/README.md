...
| Dependency update policy (Dependabot) | W2-1 -> M3 | landed | `.github/dependabot.yml`; weekly GitHub Actions, Go module, and `/web` npm version checks, patch/security grouping, no blanket ignores for Temporal/Ent/Atlas/OTel/Next; `make dependabot-policy-check` enforces the allowed ordinary version-update suppressions: linter-submodule `golang.org/x/tools` minor/major updates are coordinated with the pinned `golangci-lint` binary, `/web` `@types/node` majors track the CI Node.js baseline, and `/web` TypeScript / ESLint ordinary semver-major updates are coordinated manually with Next.js / `eslint-config-next`, generated TypeScript, lint, typecheck, build, and browser smoke gates |
