...
| ADR index, front matter, Consequences, supersession, and immutability validation | M0 -> M1 hardening | landed | `make adr-check`; validates ADR index/file consistency, requires `docs/adr` to be a direct directory, requires `docs/adr/README.md` and indexed `ADR-*.md` inputs to be regular files rather than symlinks or other non-regular paths, checks monotonic IDs, schema-valid ADR front matter (`id`, `title`, `status`, `date`, participants), accepted status values, file/H1/front-matter identity consistency, a non-empty `Consequences` section in every ADR, `supersedes` / `superseded_by` cross-reference closure, and PR/base-aware accepted ADR body immutability via `ADR_BASE_REF`. `go test ./scripts` carries fixture-backed positive and negative input-regularity / front-matter / Consequences / supersession / immutability tests. |
