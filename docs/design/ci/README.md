...
| Go toolchain version consistency | M3 hardening | landed | `make go-toolchain-check`; verifies every first-party `go.mod` uses the root patch-pinned Go directive, `.golangci.yml` uses the matching major.minor language version, every `actions/setup-go` step reads `go-version-file: go.mod` instead of a hard-coded version, and the checked `go.mod`, `.golangci.yml`, workflow directory, and workflow YAML files are direct regular inputs rather than symlinks or other non-regular files |
