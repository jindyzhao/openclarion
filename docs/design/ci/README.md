...
| Core environment boundary analyzer | W3-14 -> M2 hardening | landed | `openclarion-arch` rejects direct `os.Getenv`, `os.LookupEnv`, `os.Environ`, `os.ExpandEnv`, `os.Setenv`, `os.Unsetenv`, and `os.Clearenv` calls in non-test `internal/domain` and `internal/usecases` packages. Core alert-analysis logic must receive configuration from boundary wiring instead of reading or mutating process environment directly. |
