...
|------|---------------|--------|-------|
| docs hygiene (English-only, terminology, proof-state, and metadata freshness checks) | M0 -> M1 hardening | landed | `make docs-hygiene`; rejects non-English CJK literals, checks `docs/design/ci/terminology.tsv` only as a regular non-symlink policy file with non-symlink, directory-only parents so governed docs keep OpenClarion positioned as intelligent alert analysis, rejects generic platform positioning, restricts `SignalEvent` / `SignalGroup` / `CaseGroup` aliases to architecture boundary docs, verifies `docs/design/END_TO_END_VERIFICATION.md` only uses verdicts defined in its Verdict Scale table, and verifies `Last updated` metadata is not older than dated status/changelog table rows. |
...
| External HTTPS link inventory | M3 docs hardening | landed / scheduled | `make external-links-check`; inventories governed Markdown external HTTP(S) links locally without network access, rejecting symlink and non-regular governed Markdown inputs plus symlinked `docs/` subtrees before reading. `.github/workflows/external-links.yml` runs the opt-in live HEAD-with-GET-fallback liveness check on a weekly default-branch schedule and through `workflow_dispatch`, outside PR-blocking CI. Reserved example domains such as `.example` are inventoried but skipped during live checks. |
| Markdown structure lint | M1 hardening | landed | `make markdownlint`; runs pinned `markdownlint-cli2` from `web/package-lock.json` with regular non-symlink config `docs/design/ci/markdownlint/.markdownlint-cli2.jsonc` and non-symlink, directory-only parents, enforcing heading-level increments, unordered-list indentation, and prose line length for governed Markdown. `go test ./scripts` verifies the wrapper uses the local pinned binary, rejects indirect config files, and fails fast when dependencies are not installed. |
...
| Forbidden mutable dependency pins (go.mod, package.json, Dockerfile) | M0 -> M4 hardening | landed | `make forbidden-latest`; rejects `latest` in Go/npm manifests, requires critical first-import Go modules (Ent, Temporal SDK, OTel) to stay direct concrete root-module pins without `replace`, requires Go `tool` directive paths to resolve to concrete `require` version pins, rejects undocumented Go `replace` directives unless `docs/design/DEPENDENCIES.md` is a regular non-symlink policy file with non-symlink, directory-only parents and carries a matching `replace-allow: <module> => <target>` marker with owner and expiry, rejects `^` / `~` direct dependency ranges in first-party `package.json` files once `web/` lands, rejects `/web` `@types/node` majors that drift from the CI `actions/setup-node` major, and rejects external Dockerfile base images without an immutable `@sha256:` digest while allowing `scratch` and previous build stages. `go test ./scripts` carries Go module, Go tool directive, replace allowlist, dependency-policy regularity, Dockerfile, npm, and Node type-definition alignment fixtures. |
| Forbidden oapi-codegen v2 / openapi.compat.yaml | M0 | landed | `make forbidden-oapi-v2` |
| Forbidden SQLite in Go tests | M0 | landed | `make forbidden-sqlite`; fixture-backed script tests cover no-test skip, allowed Go tests, SQLite import/DSN rejection, non-test Go exclusion, symlink/non-regular `_test.go` fail-closed behavior, and symlinked first-party test search directories so tests cannot bypass the PostgreSQL-only rule by hiding under indirect trees |
| PR title Conventional Commit validation | M1 hardening | landed | `ci.yml` job `pr-title-check` (PR-only) calls `make pr-title-check`; validates `github.event.pull_request.title` against the Conventional Commits header shape `type(scope)!: description` with optional scope and breaking-change marker, and caps titles at 120 characters. `go test ./scripts` carries positive, negative, and 120/121-character boundary fixtures. |
...
| PR template required-section validation | M3 review-flow hardening | landed / existing job | `make pr-template-check`; the existing gate-hardening job validates `.github/pull_request_template.md` includes non-empty `## Summary`, `## Risk`, `## Rollback`, `## Local verification`, and `## DCO` sections so the template stays aligned with PR body gates without adding another top-level CI check. `make pr-template-check-test` covers missing, empty, and fenced-code false positives. |
| Issue template contract validation | M3 review-flow hardening | landed / existing job | `make issue-template-check`; the existing gate-hardening job validates bug and feature issue templates have expected front matter (`name`, `about`, `title`, `labels`) and non-empty triage sections without adding another top-level CI check. `make issue-template-check-test` covers front-matter drift, missing sections, empty placeholders, and fenced-code false positives. |
| PR impact reference validation | M3 hardening | landed / PR-only | `ci.yml` job `pr-impact-reference-check` calls `make pr-impact-reference-check`; if a PR changes `docs/adr/` or future `internal/sandbox/` paths, the PR body must link an issue or ADR using `#123`, a GitHub issue/PR URL, `ADR-0001`, or a concrete `docs/adr/ADR-0001-*.md` path. `make pr-impact-reference-check-test` covers path triggers, issue/ADR reference forms, regular-file/no-symlink/no-symlink-parent/no-non-directory-parent GitHub event JSON loading, PR diff-range fetching, missing env pairs, and no-impact bypass. |
...
| Generated file header validation | M0 -> M1 hardening | landed | `make generated-headers`; validates committed OpenAPI Go, Ent, and frontend OpenAPI TypeScript generated files retain their generator-owned headers / direct-edit warnings and rejects symlink/non-regular generated-file inputs plus symlinked or non-directory generated path parents. `go test ./scripts` carries positive, missing-header, and regular-file/no-symlink/no-symlink-parent/no-non-directory-parent fixtures for each generated family. |
...
| Go license compliance | M2 supply-chain hardening | landed | `make go-licenses-check`; runs pinned `go-licenses v1.6.0` with `--include_tests` over the root Go package set and `tools/openclarion-linter`, ignores first-party package prefixes while still checking their dependencies, and accepts only the SPDX IDs listed by `go-license-allow:` in [DEPENDENCIES.md](../DEPENDENCIES.md) when that policy path is a regular non-symlink file with non-symlink, directory-only parents. The policy marker must include non-empty owner, non-future reviewed date, and reason metadata. `go test ./scripts` covers allowlist parsing, dependency-policy regularity, command wiring, missing policy markers, missing metadata, invalid/future reviewed dates, and tool failure propagation. |
...
| Go lint baseline (golangci-lint) | W1-3 | landed | `make go-lint`; bootstraps `github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.2` into ignored `bin/`, runs the custom analyzer plugin, and enables `unused` plus `ineffassign` so dead package-scope helpers and ineffectual assignments cannot accumulate silently; CI job `go-lint` |
| Dependency update policy (Dependabot) | W2-1 -> M3 | landed | `.github/dependabot.yml`; weekly GitHub Actions, Go module, and `/web` npm version checks, patch/security grouping, no blanket ignores for Temporal/Ent/Atlas/OTel/Next; linter-submodule `golang.org/x/tools` minor/major updates are coordinated with the pinned `golangci-lint` binary, `/web` `@types/node` majors track the CI Node.js baseline, and `/web` TypeScript / ESLint ordinary semver-major updates are coordinated manually with Next.js / `eslint-config-next`, generated TypeScript, lint, typecheck, build, and browser smoke gates. `make dependabot-policy-check` parses `.github/dependabot.yml` with `go.yaml.in/yaml/v3`, validates known fields, and proves the linter/web ignore plus security-update group invariants. |
...
| OpenAPI critical-node fingerprint lock | W4-2 -> M3/M5 | landed | `make openapi-fingerprint`; lock file `docs/design/ci/locks/openapi-critical.lock` generated from the real `api/openapi.yaml` critical domain API nodes, now including dashboard, alert, evidence, report read paths, report replay trigger path, diagnosis room creation, and diagnosis WebSocket ticket issuance. The spec reader rejects symlink or non-regular OpenAPI spec inputs plus duplicate-key, multi-document, anchor, merge-key, or alias YAML before hashing; the lock parser rejects symlink or non-regular lock inputs plus duplicate-key or trailing-value JSON before comparing fingerprints; and update mode refuses indirect existing lock paths, including symlinked parent directories. |
...
| Gate hardening maturity checklist | M3 hardening | landed | `make gate-hardening-check`; validates [GATE_HARDENING_CHECKLIST.md](GATE_HARDENING_CHECKLIST.md) covers every activated Progressive Gate Schedule row exactly once with an allowed maturity level, concrete evidence, and a concrete next-hardening note, and rejects symlink/non-regular schedule or checklist inputs, including symlinked parent directories, before parsing |
| Tracked text file hygiene | M3 hardening | landed / existing job | `make text-file-hygiene`; validates tracked text-like files from `git ls-files` use valid UTF-8 without BOM, LF line endings, no NUL bytes, and no hidden C0 / DEL control bytes outside tab and newline. CI runs it in the existing Docs and text hygiene job instead of adding another top-level workflow check. |
| Git tracked file mode policy | M3 hardening | landed / existing job | `make file-mode-check`; validates `git ls-files -s -z` mode bits so regular tracked files stay `100644`, executable bits are limited to direct `scripts/*.sh` entrypoints, and tracked symlinks / submodules / unknown modes are rejected before they become CI inputs. CI runs it in the existing hygiene job instead of adding another top-level workflow check. |
...
| Shell script syntax validation | M3 hardening | landed | `make shell-syntax-check`; discovers Git-tracked `.sh` files and shell-shebang scripts, requires `.sh` entries to be direct regular files, uses bounded first-line reads for shebang detection, and runs `bash -n` through a fixture-backed Go harness before scripts can enter CI. |
| YAML configuration syntax validation | M3 hardening | landed | `make yaml-syntax-check`; parses every Git-tracked `.yml` / `.yaml` file with `go.yaml.in/yaml/v3`, rejects symlink/non-regular YAML inputs, enforces a bounded file size, requires a single-document scalar-key YAML tree, and rejects duplicate keys, anchors, aliases, and merge keys before configuration drift can hide behind parser-specific overwrite or expansion behavior. |
| Source comment debt tracking | M3 hardening | landed | `make comment-debt-check`; scans supported source comments for debt markers (`TODO`, `FIXME`, `HACK`, `XXX`) and requires a tracking reference using `#123`, a GitHub issue/PR URL, `ADR-0001`, or a concrete `docs/adr/ADR-0001-*.md` path. `make comment-debt-check-test` covers Go parser-backed comment scanning, string-literal avoidance, generated/testdata/ignored-path skipping, supported-path symlink rejection, non-Go comment prefixes, marker boundaries, and violation output. |
| Deferred follow-up ledger validation | M3 governance hardening | landed | `make deferred-followups-check`; validates regular-file [DEFERRED_FOLLOWUPS.md](../DEFERRED_FOLLOWUPS.md) input and entries with contiguous unique `D<number>` IDs, canonical `open` / `closed` status, section/status consistency, required concrete fields, valid non-future decision/update dates, and no unknown table fields. Closed D6/D11 entries now live under Closed Deferrals. |
| Allowlist discipline | M3 hardening | landed | `make allowlist-discipline`; validates repository allowlist entries carry adjacent `Owner`, `Expires`, and `Removal trigger` comments, rejects expired entries, and caps review expiry at 120 days so false-positive suppressions remain reviewable and temporary. `make allowlist-discipline-test` covers missing metadata, detached metadata, expired entries, overlong review windows, same-day expiry, horizon-boundary expiry, read-error handling, and regular-file/no-symlink/no-symlink-parent/no-non-directory-parent allowlist input loading. |
| Branch protection required-check policy | M3 hardening | landed / existing job | `make branch-protection-check`; validates [branch-protection.required-checks.json](branch-protection.required-checks.json) as the repository policy source for `main` required status checks, requiring a strict GitHub Actions source policy whose contexts exactly match the unique job names in PR-triggered workflows. `make branch-protection-check-test` covers missing, stale, duplicate, and unsorted contexts, duplicate workflow job names, non-PR workflow exclusion, duplicate JSON/YAML keys, and non-regular policy files. CI runs these targets in the existing Workflow / Makefile parity job instead of adding another top-level workflow check. |
...
| Linear history guard | M3 review-flow hardening | landed / PR-only existing job | `make linear-history-check`; the existing DCO PR job checks PR ranges for merge commits so branch review history stays rebase-oriented without adding another top-level CI check. `make linear-history-check-test` covers PR range selection, missing-base fetch behavior, missing SHA failures, upstream/local fallbacks, and merge-commit rejection. |
| PR file count cap | M3 review-flow hardening | landed / PR-only existing job | `make pr-file-count-check`; the existing PR impact reference job reads GitHub's `pull_request.changed_files` count plus PR labels from `GITHUB_EVENT_PATH`, enforces the default 50-file review cap, and allows the maintainer label `large-pr-approved` as an explicit override without adding another top-level CI check. `make pr-file-count-check-test` covers event JSON, env/local inputs, PR-range fallback, invalid config, over-limit rejection, and override acceptance. |
...
| Repository size budget | M3 hardening | landed | `make repo-size-check`; scans Git-visible tracked and non-ignored untracked working-tree files through `git ls-files`, rejects symlink/non-regular paths, enforces the default 1 MiB per-file / 25 MiB total budget, and rejects common archive, executable, object, packaged JVM, WebAssembly, and database artifact extensions before large retained evidence, generated blobs, or accidental binary files enter review. `make repo-size-check-test` covers sorting, unsafe paths, symlinks, blocked artifact extensions, per-file failures, total failures, and invalid budgets. CI runs these targets in the existing PR budget job instead of adding another top-level workflow check. |
...
| LLM output acceptance tests | M2 | landed | `go test ./internal/usecases/llmoutput`; covers refusal, truncation / non-`stop` finish reasons, invalid JSON, duplicate-key / trailing-value strict JSON rejection for provider content and output schemas, JSON Schema violations, unsupported output modes, and `json_object` fallback validation |
...
| LLM report draft schema tests | M2 | landed | `go test ./internal/usecases/reportdraft`; covers SubReport/FinalReport valid parse, missing/extra/invalid enum rejection, local semantic bounds, duplicate `evidence_refs` rejection, finding-to-`evidence_refs` binding, defensive schema copies, and OpenAI strict structured-output compatibility (`additionalProperties:false`, all properties required, root object schemas, approved keyword subset only) |
| OpenAI-compatible LLM provider tests | M2 | landed | `go test ./internal/providers/llm/openai`; covers Chat Completions request shape for strict `json_schema`, `json_object` fallback request shape, refusal mapping, API error wrapping, ambiguous/oversized response-envelope rejection, plain-text API errors, invalid request preflight including non-HTTP(S) and userinfo-bearing base URLs, strict capability probe success, strict-unsupported fallback, ambiguous capability-error rejection, and non-capability probe error propagation |
| Report persistence repository tests | M2 | landed | `go test ./internal/domain ./internal/persistence/repository`; covers SubReport/FinalReport constructors, duplicate-key-free retained report/delivery JSON, per-snapshot SubReport idempotency, global FinalReport idempotency, FinalReport-to-SubReport fan-in links, notification delivery pending/delivered/failed persistence, newest-first list queries, and repository invariant errors |
| Report Temporal workflow tests | M2 | landed | `go test ./internal/orchestrator/temporal`; covers ReportFanOutWorkflow/ReportBatchWorkflow/FinalReportWorkflow scheduling, batch parent-child fan-out/fan-in, completed-history replay via `worker.NewWorkflowReplayer`, GenerateSubReport/GenerateFinalReport persistence and idempotency with a fake LLM provider, and SendReportNotification delivery-log persistence / duplicate delivered skip behaviour before and after IMProvider calls |
...
| IM provider tests | M2/M5 | landed | `go test ./internal/providers/im/...`; covers deterministic fake provider scripts/copy isolation and Webhook JSON POST shape, idempotency headers, FinalReport and DiagnosisTask notification subjects, optional Bearer auth, response parsing, ambiguous non-object/duplicate/trailing response rejection, oversized response rejection, retryable status classification, userinfo-bearing webhook URL rejection, and invalid input/config handling |
...
| Runtime provider wiring tests | M2/M5 | landed | `go test ./cmd/openclarion`; covers env-driven OpenAI-compatible LLM provider injection, Webhook IM provider injection, Prometheus trigger wiring, diagnosis-room OIDC/ticket/Temporal relay/starter wiring, Docker-backed diagnosis sandbox provider wiring, browser origin/CORS allowlist parsing including userinfo-bearing allowed-origin rejection, CLI one-shot argument validation, unconfigured startup compatibility, and partial-config fail-fast behavior |
...
| Agent runtime dependency and hardcoding boundary | M4 | landed | `make forbidden-agent-runtime`; reads regular-file `docs/design/ci/agent-runtime-forbidden.tsv`, validates the policy table for scope coverage, duplicate rows, and unpadded patterns, rejects the listed runtime-family dependencies from structured `go.mod` require/replace entries plus `package.json` dependency sections, and rejects those names in non-test Go control-plane import paths, string literals, and identifiers plus non-test shell/frontend source under `cmd/`, `internal/`, `scripts/`, and `web/src/` until [agent-runtime-selection.md](../agent-runtime-selection.md) accepts a sandbox runtime baseline. The policy is configurable governance data, not production code binding; documentation, tests, Go comments, and non-dependency `package.json` fields remain available for candidate evidence. |
...
| Sandbox baseline audit | M4 | landed | `make sandbox-baseline-audit`; emits a code-level JSON proof for ADR-0013 file paths, network-none batch defaults, M5 turn input mounts, Docker security posture, resource limits, allowlist subset enforcement, and strict request/raw-output JSON validation. Manual `make sandbox-m4-baseline-audit OUT=...` writes the same proof to a retained evidence file without overwriting existing output. It complements manual Docker smokes rather than replacing them. |
| Sandbox M4 quality sample export | M4 | landed / manual | `make sandbox-m4-quality-sample-export`; exports operator-selected persisted direct/sandbox SubReport rows from PostgreSQL into the retained `direct/<scenario>/<case>.json` and `sandbox/<scenario>/<case>.json` sample-root layout. It rejects ambiguous selection JSON, duplicate case/report IDs, scenario mismatches, invalid persisted SubReport JSON, mixed evidence snapshots, symlinks, non-regular selection files, and non-empty output roots before handing samples to manifest preparation. It does not choose representative samples, run model inference, compare quality, or accept a runtime candidate. |
...
| Sandbox M4 decision gate tests | M4 | landed | `make sandbox-m4-decision-test`; covers the offline proceed/iterate/defer decision helper that combines baseline audit output, manifest-mode quality comparison output, runtime smoke evidence, and human review evidence while rejecting symlink/non-regular evidence files, duplicate JSON object keys, unknown fields, and duplicate baseline audit check names in evidence inputs. Quality comparison evidence must cover `single_alert`, `cascade`, and `alert_storm`; quality `sample_basis` must be required, single-line, unpadded, and bounded to 2048 bytes; every case must retain single-line bounded IDs and non-empty single-line bounded `required_evidence_refs` with a canonical `snapshot:<positive-id>` EvidenceSnapshot ref; aggregate summary counts must match `cases[].recommendation`; review evidence `sample_basis` must match the quality comparison `sample_basis`; review evidence must cover the same single-line bounded case IDs via `reviewed_cases`; `selected_candidate` and `candidate_evaluations` are generic unpadded whitespace-free evidence-supplied IDs, with the selected candidate required to have a matching `pass` evaluation whose immutable `runtime_candidate` equals the top-level digest-pinned `runtime_candidate` and whose `runtime_smoke_refs` cite every required runtime smoke name; loopback or `localhost` registry references remain local smoke evidence and force `defer` instead of supporting `proceed`; review evidence human-authored text is required where applicable, single-line, unpadded, and bounded to 2048 bytes; runtime smoke names must be exactly the required smoke set, sources must match their canonical `make` targets, and each smoke must carry a distinct bounded normalized relative `evidence_ref` plus lowercase `evidence_sha256`, preventing free-form pass claims or local-machine-specific paths from satisfying the decision gate. If incomplete evidence and explicit failure evidence coexist, the decision remains `defer` but retained reasons include both classes. The manual `make sandbox-m4-decision` target requires real evidence files and is not auto-run in CI. |
| Sandbox M4 review-evidence template | M4 | landed / manual | `make sandbox-m4-review-evidence-template`; builds a draft `sandbox_m4_review_evidence` JSON file from manifest-mode quality comparison output and retained runtime-smoke artifacts. It reads direct regular files only, rejects duplicate JSON object keys in source artifacts, copies the quality `sample_basis` and case IDs into `reviewed_cases`, fills canonical runtime-smoke names/sources, reads each smoke artifact status, and records SHA-256 digests for the retained smoke files. Operators can provide either `RUNTIME_CANDIDATE=<image@sha256:digest>` or `RUNTIME_CANDIDATE_FILE=<digest-ref.txt>` for custom thin runner artifact handoff; both paths preserve immutable image-reference validation. When `OUT=...` is set, the output path must be absent before the helper writes the draft, preventing retained review evidence from being overwritten in place. It is fail-closed by default: candidate evaluation, reviewed-case, and human-review statuses are `fail`, and `representative_sample` is false unless `REPRESENTATIVE_SAMPLE=1` is explicitly set. The target is manual and does not accept an M4 runtime baseline. |
| Sandbox M4 evidence packet tests | M4 | landed | `make sandbox-m4-evidence-packet-test`; covers the manual packet assembler and offline retained-packet verifier. Assembly freezes baseline audit, manifest quality comparison, copied quality manifest and raw direct/sandbox SubReport inputs, copied review evidence, copied runtime-smoke artifacts, decision output, and packet summary into one empty evidence directory. The assembler rejects weak artifacts before writing them: baseline output must contain uniquely named required pass checks, quality output must be manifest-mode with required single-line bounded sample basis, required scenario coverage, unique single-line bounded case IDs, per-case review markers, per-case single-line bounded required evidence refs, and summary counts derived from case recommendations; copied quality manifest sample basis, case IDs, scenarios, and required refs must match the generated comparison output; copied direct/sandbox SubReports must pass the production SubReport parser and contain every declared required evidence ref; review evidence must contain dated representative-sample/runtime-smoke/human-review fields with generic unpadded whitespace-free `selected_candidate` / `candidate_evaluations`, a digest-pinned `runtime_candidate`, a matching selected-candidate evaluation runtime ref, selected pass-candidate `runtime_smoke_refs` for every required runtime smoke name, reviewer/status/notes metadata, exactly required runtime smoke names with canonical sources, distinct bounded normalized relative runtime-smoke `evidence_ref` values and lowercase `evidence_sha256` digests, case-level single-line bounded `reviewed_cases`, and the same sample basis as the generated quality comparison, duplicate JSON object keys and unknown fields must be absent, human-authored review text must be single-line, unpadded, and bounded to 2048 bytes, runtime-smoke source artifacts must exist under the configured artifact root, be regular files rather than symlinks or other non-regular files, and match their declared SHA-256 digests before being copied into packet-local `runtime-smoke-artifacts/` paths, decision output must use a valid proceed/iterate/defer value with decision-consistent non-blank, non-duplicate reasons, must not use a loopback registry runtime candidate for `proceed`, and decision evidence must match the frozen baseline/quality/review inputs plus `--min-cases`, selected-candidate summary, reviewed-case count, and reviewed case IDs. The retained `packet.json` records `out_dir` as `.`, stores generated artifact and helper output paths as packet-local slash-separated paths, and records SHA-256 digests for the frozen baseline, quality, review, decision, quality-input, and runtime-smoke artifacts. Verification mode `make sandbox-m4-evidence-packet-verify PACKET_DIR=...` revalidates an existing packet without rerunning helpers and rejects stale command metadata, digest drift, missing copied inputs, quality manifest/output drift, copied report schema/ref failures, runtime-smoke artifact drift, decision evidence mismatches, unexpected packet files/directories, symlinks, non-regular files, and `PACKET_JSON` aliases that do not match the packet's own `artifacts.packet` reference. |
| Sandbox M4 evidence-chain readiness | M4 | landed / manual | `make manual-evidence-readiness --target sandbox-m4-evidence-chain`; checks an operator-selected retained M4 evidence root via `OPENCLARION_M4_EVIDENCE_ROOT` and reports whether the canonical baseline audit, runtime-smoke artifacts, digest-ref handoff, direct/sandbox sample directories, quality manifest, quality comparison, review evidence, and packet summary are present. Existing JSON artifacts are checked for duplicate object keys and trailing JSON values before their SHA-256 digests are reported. The output intentionally uses only canonical artifact names, statuses, reasons, and digests, not local paths. This is a gap audit and does not select representative samples, compare quality, run Docker, or accept a runtime baseline. |
| Diagnosis room policy tests | M5 | landed | `make diagnosis-room-policy-test`; covers the pure short-conversation policy boundary for configured turn/time/message/context limits, duplicate message IDs, in-flight turn rejection, basic unsafe-instruction denylist matching, duplicate-key and trailing-value evidence JSON rejection before context byte accounting, and strict V1 sandbox `output.json` schema validation, including duplicate-key and trailing-value rejection before JSON Schema validation. |
| Diagnosis room workflow tests | M5 | landed | `make diagnosis-room-workflow-test`; covers the Temporal room control plane plus per-turn sandbox, transcript persistence, lifecycle audit, and close-notification Activity boundaries: room workflow start options/readiness query through `DiagnosisRoomStarter`, duplicate-key-free JSON-object evidence rejection before task/session creation, `submit-turn` Update with policy Validator, `EnsureDiagnosisRoomSession`, `EnsureDiagnosisChatSession`, `RunDiagnosisTurn`, `PersistDiagnosisTurn`, `CloseDiagnosisChatSession`, `SendDiagnosisRoomCloseNotification`, `ContainerProvider.Run` request construction, strict-JSON and schema-valid assistant output acceptance, idempotent user+assistant ChatTurn persistence, idempotent opened/turn-persisted/closed/close-notification `DiagnosisTaskEvent` audit rows, duplicate-key and trailing-value lifecycle audit payload rejection before event persistence, terminal `ChatSession` close metadata, idempotent IMProvider close notification, workflow-state transcript updates after persistence, `state` Query, close/cancel Signals, durable idle/session timers, duplicate `message_id` rejection, concurrent Update rejection while an Activity is in flight, configured max-turn rejection, unsafe-message rejection, and Temporal client start/Update/Query option mapping. |
| Diagnosis auth boundary tests | M5 | landed | `make diagnosis-auth-test`; covers provider-neutral AuthProvider DTOs, OIDC ID-token verification, OIDC issuer URL userinfo rejection, malformed/trailing claim-payload rejection during verification, duplicate claim-name rejection after verification and before role-claim mapping, deterministic auth fake scripts, owner/admin RBAC, short-lived single-use WebSocket ticket issuance/consumption, TTL expiry, wrong-session rejection, defensive in-memory store copies, PostgreSQL-backed ticket persistence with hashed tokens plus concurrent single-consume protection, HTTP/WS transport handshake coverage including malformed Origin rejection before ticket consumption, authenticated `submit_turn` / `query_state` relay, bounded Update timeout error frames, and disconnect behavior that does not cancel submitted Updates. |
...
| Diagnosis dev OIDC issuer | M5 | manual | `make diagnosis-dev-oidc-issuer`; serves loopback-only OIDC discovery and JWKS metadata plus a short-lived local operator ID-token endpoint for M5 live-smoke setup. It is not run in CI and does not satisfy M5 acceptance by itself; the runtime still verifies the token through `internal/providers/auth/oidc`, and the live browser proof still requires a real persisted `EvidenceSnapshot`, Temporal worker, sandbox provider, and retained `make diagnosis-live-browser-smoke` output. |
| Docker Provider live smoke harness | M4 | landed / manual | `make container-provider-smoke`; runs `ContainerProvider.Run` through a real local Docker daemon, digest-pinned image ref, network-none, non-root user, readonly rootfs, readonly ADR-0013 input mounts, `/workspace/out` as the only writable mount with `fsize` cap, Docker output archive copy, duplicate-key-free JSON-object output validation, optional `OPENCLARION_CONTAINER_PROVIDER_SMOKE_PROOF_PATH` proof JSON with image ref, source target, output byte count, output SHA-256, and passed checks, plus invocation-label leak check. Not run in CI because it requires a local Docker daemon. |
| Docker Provider timeout cleanup smoke | M4 | landed / manual | `make container-provider-timeout-smoke`; runs the same Docker-backed Provider with a short timeout and long-running command, expects `context deadline exceeded`, can write the same proof JSON in expected-error mode, then fails if any invocation-labelled container remains. Not run in CI because it requires a local Docker daemon. |
| Docker Provider output cap smoke | M4 | landed / manual | `make container-provider-output-cap-smoke`; runs the same Docker-backed Provider with a 64-byte output cap and a command that writes more than the cap, accepts either `fsize`-enforced process failure or Provider output-size rejection, can write the same proof JSON in expected-error mode, then fails if any invocation-labelled container remains. Not run in CI because it requires a local Docker daemon. |
| Egress allow-deny smoke harness | M4 | landed / manual | `make egress-allowdeny-smoke`; resolves the helper image to a digest-pinned ref, creates Docker sandbox/upstream networks plus an allowlist proxy, proves allowed-via-proxy succeeds, denied-via-proxy returns 403, and direct sandbox bypass fails. When `OPENCLARION_EGRESS_SMOKE_PROOF_PATH` is set, writes a retained proof JSON with source, digest-pinned image ref, run id, timeout, topology, and passed allow/deny/bypass/security checks. Not run in CI because it requires a local Docker daemon and mutable Docker networks. |
| Manual agent runtime adapter smoke harness | M4 | landed / manual | `make agent-runtime-smoke`; runs a digest-pinned candidate image through network-none, non-root, readonly rootfs, `no-new-privileges`, dropped capabilities, memory/CPU/PID limits, readonly ADR-0013 input mounts, writable `/workspace/out` output mount with `fsize` cap, timeout cleanup, regular-file/no-symlink output-artifact validation, bounded reads, duplicate-key-free non-empty JSON-object output validation, and a retained runtime-agnostic proof JSON containing the source target, digest-pinned runtime candidate, container output path, output byte count, configured cap, output SHA-256, and passed checks without host-local paths. |
| Custom thin runner candidate smoke | M4 | landed / manual | `make custom-thin-runner-smoke`; builds the local scratch-based custom runner plus packaged metric/topology helper binaries, pushes it to an ephemeral localhost registry to obtain a real `repo@sha256` reference, proves the packaged topology helper with `--entrypoint`, then runs both `make agent-runtime-smoke` and `make container-provider-smoke`. When `OPENCLARION_CUSTOM_THIN_RUNNER_ARTIFACTS_DIR` is set, the target also runs the existing runtime-smoke artifact bundle while that registry is alive. The runner rejects symlink/non-regular ADR-0013 JSON input paths plus duplicate-key or trailing-value ADR-0013 input JSON before hashing mounted inputs. This is a contract/lifecycle proof, not a report-quality baseline. |
...
| WebSocket auth handshake and relay test | M5 | landed | covered by `make diagnosis-auth-test`; verifies `POST /api/v1/diagnosis/ws-ticket`, ticket-required `GET /ws/diagnosis`, non-upgrade rejection before ticket consumption, malformed/cross-host Origin rejection before ticket consumption, authenticated connection handoff, `submit_turn` to Temporal Update, `query_state` to Temporal Query, timeout response, and disconnect behavior |
...
| Audit completeness test | M5 | landed | covered by `make diagnosis-room-workflow-test`; verifies opened, turn-persisted, closed, and close-notification lifecycle events are idempotently logged through `DiagnosisTaskEvent`, and that ambiguous lifecycle payload JSON is rejected before append-only audit persistence |
...
make actionlint    # GitHub Actions workflow syntax and semantic lint
make docs-hygiene  # documentation language, terminology, proof-state, and metadata freshness gates
...
make text-file-hygiene # tracked text encoding and line-ending hygiene
make file-mode-check # tracked Git file mode policy
make comment-debt-check # source comment debt tracking
make deferred-followups-check # deferred decision ledger
make shell-syntax-check # tracked shell script syntax validation
...
make pr-template-check # PR template required-section policy
make issue-template-check # issue template front matter and required sections
...
make sandbox-m4-baseline-audit # manual M4 retained baseline audit output
make sandbox-quality-compare-test # M4 offline direct-vs-sandbox SubReport comparison tests
make sandbox-m4-quality-sample-export # manual M4 quality sample export from persisted SubReport rows
make sandbox-m4-quality-manifest-prepare # manual M4 quality manifest preparation from retained report pairs
make sandbox-m4-quality-compare # manual M4 retained quality comparison from manifest
...
OPENCLARION_M4_EVIDENCE_ROOT=... make manual-evidence-readiness --target sandbox-m4-evidence-chain # manual M4 evidence-chain gap audit
...
| `.github/workflows/external-links.yml` | Weekly/manual live external-link liveness check on the default branch; non-PR-blocking |
...
| Tightening | Trigger | Implementation hint |
|---|---|---|
| Squash/rebase merge strategy as repository settings-as-code | M1 | repo setting management plus a guard that does not conflict with the chosen merge method |
