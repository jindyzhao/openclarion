...
| Custom analyzer scaffold + version parity | W3-1 -> M3 | landed | `make go-lint` now runs `tools/openclarion-linter` tests, validates `golang.org/x/tools` parity via `scripts/check_lint_version.sh`, rejects symlink/non-regular lint-version inputs, builds `.custom-gcl.yml` with `golangci-lint custom`, and runs `bin/custom-gcl` over the root-module Go package set |
