...
| Agent runtime dependency and hardcoding boundary | M4 | landed | `make forbidden-agent-runtime`; reads `docs/design/ci/agent-runtime-forbidden.tsv`, validates the policy table for scope coverage, duplicate rows, and unpadded patterns, rejects the listed runtime-family dependencies from structured first-party manifest dependency fields (`package.json` dependency maps plus `go.mod` `require` / `tool` directives parsed with `golang.org/x/mod/modfile`), and rejects those names in non-test Go control-plane source, until [agent-runtime-selection.md](../agent-runtime-selection.md) accepts a sandbox runtime baseline. The policy is configurable governance data, not production code binding. |
