...
| OpenAPI breaking-change diff | W4-1 | landed / soft-fail until 2026-06-10 | `make openapi-breaking`; oasdiff `v1.11.7`; soft-fail owner: CI maintainers; after 2026-06-10 the script hard-fails automatically. Root `scripts` tests cover pre-sunset soft-fail, on/after-sunset hard-fail, non-breaking success, invalid sunset config, invalid current-date config, symlink/non-regular current or explicit base specs, and non-regular git base objects before diffing. |
