...
| Runtime provider wiring tests | M2/M5 | landed | `go test ./cmd/openclarion`; covers env-driven OpenAI-compatible LLM provider injection, Webhook IM provider injection, Prometheus trigger wiring, diagnosis-room OIDC/ticket/Temporal relay/starter wiring, Docker-backed diagnosis sandbox provider wiring, browser origin/CORS allowlist parsing including userinfo rejection, CLI one-shot argument validation, unconfigured startup compatibility, and partial-config fail-fast behavior |
