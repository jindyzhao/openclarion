...
| PR commit-shape validation | M3 hardening | landed / PR-only | `ci.yml` job `pr-commit-shape-check` calls `make pr-commit-shape-check`; rejects merge commits in the PR base/head range so branches stay rebaseable and repository merges remain squash-only. `make pr-commit-shape-check-test` covers PR range selection, missing-base fetch, missing env pairs, local `HEAD^!` fallback, deterministic merge-commit diagnostics, and malformed git-log output. |
...
| Repository squash-merge-only setting recorded as policy | when repository settings are managed as code | configure GitHub merge settings and keep `make pr-commit-shape-check` as the PR-side defense-in-depth guard |
...
| Keep `make pr` as the primary reproducible local CI command, with explicit documentation for PR-context-only exceptions | always | DCO, PR metadata checks, PR impact references, workflow-change isolation, and PR commit-shape validation are current exceptions because their authoritative input exists only on `pull_request`; record any new deviation as a deliberate ADR |
