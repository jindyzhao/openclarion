...
| External HTTPS link inventory | M3 docs hardening | landed / scheduled | `make external-links-check`; inventories governed Markdown external HTTP(S) links locally without network access. `.github/workflows/external-links.yml` runs the opt-in live HEAD-with-GET-fallback liveness check on a weekly default-branch schedule and through `workflow_dispatch`, outside PR-blocking CI. Reserved example domains such as `.example` are inventoried but skipped during live checks. |
