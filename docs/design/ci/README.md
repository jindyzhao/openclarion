...
| Dependency update policy (Dependabot) | W2-1 -> M3 | landed | `make dependabot-policy-check`; validates `.github/dependabot.yml` is a direct regular single-document file with four weekly update rules for GitHub Actions, root Go, linter-tooling Go, and `/web` npm; each rule must keep fixed schedule timing/timezone, expected labels, separated patch/security groups, open-PR caps, and no blanket ignores. The only allowed ignore is the linter-submodule `golang.org/x/tools` ordinary minor/major version-update exception coordinated with the pinned `golangci-lint` binary because `make go-lint` enforces exact plugin parity. |
...
make dependabot-policy-check # Dependabot update policy shape
