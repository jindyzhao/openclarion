...
| Docker Engine sandbox provider | Docker Go SDK modules `github.com/moby/moby/api v1.54.1` and `github.com/moby/moby/client v0.4.0` (direct requires since `internal/providers/container/docker/provider.go` imports Engine API types and the official client for create/start/wait/stop/kill/remove/copy lifecycle calls; unit tests use a fake EngineClient so cleanup, timeout, and output-copy behavior are verified without requiring a local daemon) | 2026-05-28 |
