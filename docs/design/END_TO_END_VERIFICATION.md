...
| A9 | Send notification | `FinalReportWorkflow` schedules `SendReportNotification` only after `GenerateFinalReport` returns a persisted ID; the Activity can select either the legacy env-injected IMProvider or a profile-backed Webhook/Email provider for a bound report notification channel, persists a pending delivery row before calling the selected provider, marks it delivered/failed afterwards, skips duplicate provider calls after delivered, and the Webhook provider posts either default OpenClarion JSON with idempotency headers or supported robot / incoming-webhook text envelopes while Email renders plain-text SMTP notifications | proven-local |
