...
type exists. The proof projection can show policy replay and scheduled trigger
as separate retained evidence targets, but it must not store a browser-local
wizard state or treat nonzero counts as proof that connections, previews,
enablement, schedule reconciliation, or live delivery have succeeded.
