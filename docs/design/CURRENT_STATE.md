...
> Stage: M1-PR5 in progress (deterministic grouping algorithm —
> pure function `GroupEvents` that buckets persisted `AlertEvent`s by
> configurable dimension keys, computes sha256 group key from canonical
> dimensions JSON, aggregates max severity, and returns sorted
> `AlertGroup` drafts with stable `EventIDs`; layered on top of M1-PR4
> which shipped `MetricsProvider` port + Prometheus + fake +
> `alertingest.IngestOnce` ingestion library).
...
| 2026-05-26 | jindyzhao | M1-PR4 shipped. M1-PR5 in progress: deterministic grouping algorithm. Pure function `GroupEvents(events []domain.AlertEvent, cfg Config) ([]domain.AlertGroup, error)` in `internal/usecases/alertgrouping/` -- configurable `DimensionKeys` (default `["alertname"]`) + `SeverityKey` (default `"severity"`); validates config (trim + dedup + sort) and input (non-zero ID + non-zero StartsAt); buckets events by canonical dimensions JSON; output `GroupKey` = sha256 hex; severity = max across group (trim+lower normalised); `EventIDs` sorted `(StartsAt asc, ID asc)`; groups sorted `(FirstSeenAt asc, GroupKey asc)`. 19 unit tests cover determinism, severity promotion/normalization, boundary errors, config dedup/order-invariance. No persistence orchestration -- deferred to a subsequent PR. |
