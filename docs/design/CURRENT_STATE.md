...
> Stage: M1-PR4 in progress (MetricsProvider port + Prometheus HTTP
> API v1 implementation + in-memory fake provider + `IngestOnce`
> AlertEvent ingestion library, layered on top of the M1-PR3
> baseline which shipped post-merge as `3874f44` -- Temporal Go SDK
> first-import pin, `DiagnosisWorkflow` shell, `RecordDiagnosisEvent`
> activity, and Update / Signal round-trip integration tests).
> Earlier M1-PR2 landed domain constructors with cross-field
> invariants (`AlertEvent`, `AlertGroup`, `EvidenceSnapshot`,
> `DiagnosisTask`, `DiagnosisTaskEvent`), three aggregate-root
> repository contracts (`AlertRepository`, `EvidenceRepository`,
> `DiagnosisRepository`) behind `UnitOfWork`, and the Ent-backed
> implementations with full integration tests.
