...
> Stage: M1-PR6 in progress (EvidenceSnapshot deterministic builder --
> pure function `BuildSnapshot` that packs an `AlertGroup` + its
> `AlertEvents` into a canonical `EvidenceSnapshot` with sha256 digest,
> static provenance, and complete status; layered on top of M1-PR5
> which shipped the deterministic grouping algorithm `GroupEvents`).
...
| EvidenceSnapshot builder | in progress (M1-PR6) | Pure function `BuildSnapshot(Input) (EvidenceSnapshot, error)` in `internal/usecases/evidencebuild/`; deterministic payload JSON with canonical nested `json.RawMessage`, sha256 digest, static provenance (`openclarion.core`), status=complete. 16-point input validation; no provider calls, no persistence orchestration. |
...
| 2026-05-26 | jindyzhao | M1-PR5 shipped. M1-PR6 in progress: EvidenceSnapshot deterministic builder. Pure function `BuildSnapshot(Input) (domain.EvidenceSnapshot, error)` in `internal/usecases/evidencebuild/` -- packs `AlertGroup` + `[]AlertEvent` into a canonical snapshot payload (typed structs for field-order stability, all nested `json.RawMessage` canonicalized via unmarshal+remarshal, nil labels/annotations normalized to `{}`, `*json.RawMessage` for null rendering). Digest = sha256 of the marshalled canonical payload bytes. Provenance = static `openclarion.core` with `status:ok, inputs:[alert_group, alert_events]`. Status = complete, MissingFields = nil. 16-point input validation: zero IDs, empty strings, zero/invalid times, LastSeenAt < FirstSeenAt, invalid severity, invalid/non-object dimensions, non-valid RawPayload, event outside time range, duplicate event IDs, EventIDs set mismatch. `CreatedByWorkflow` flows to the domain struct but is NOT part of payload/digest. All times normalized via `domain.NormalizeUTCMicro` before comparison and formatting. 33 unit tests cover determinism, canonical JSON key-order/whitespace invariance, nil rendering, validation boundaries, provenance/status, and digest stability. No provider calls, no persistence orchestration, no changes to `cmd/openclarion/main.go`. |
